...
exports.create = async (req, res) => {
    if (!req.body.email && !req.body.firstName && !req.body.lastName && !req.body.address) {
        //res.status(400).send({ message: "Content can not be empty!" });
        res.status(400).render('results', {mydata: "Content can not be empty!"})
    }

    const user = new OperationModel({
    ...
    await user.save().then(data => {
        ...
        res.status(200).render('results', {mydata: "user "+ data.email +" created succesfully!"})
    }).catch(err => {
        /*res.status(500).send({
...
exports.findAll = async (req, res) => {
    try {
        const user = await OperationModel.find();
...
exports.findOne = async (req, res) => {
    try {
        const user = await OperationModel.findOne({email: req.query.email}).exec(); //change params to query
        ...
        if (user===null){
            ...
            res.status(200).render('results', {mydata: "user :"+ user.firstName +" "
                    + user.lastName +" "+ user.email +" "+ user.address
            })
        }
...
exports.update = async (req, res) => {
    ...
    if (!req.body.newEmail || !req.body.newFirstName || !req.body.newLastName || !req.body.newAddress) {
        //res.status(400).send({ message: "Content can not be empty!" });
    ...
    await OperationModel.findOneAndUpdate({email: query}, {email:req.body.newEmail,
        firstName:req.body.newFirstName,
        lastName:req.body.newLastName,
        phone:req.body.newAddress
    }).then(data => {
        console.log(data)
        if (!data) {
...
exports.destroy = async (req, res) => {
    ...
    await OperationModel.deleteOne({email: useremail}).then(data => {
